...
if __name__ == "__main__":
    ...
    # exist samp folder
    FILE_NUM  = len(list(filter(lambda x: x.startswith('samp0'), os.listdir(DATASET+'/sampled_data/'))))
    for fi in range(1, FILE_NUM+1):
        samp_path = DATASET + '/sampled_data/' + 'samp0' + str(fi) + '/'
        if not os.path.exists(samp_path):
                os.mkdir(samp_path)


        if os.path.exists( samp_path + '/user_profile_' + str(FRAC) + '.pkl') and os.path.exists( samp_path + '/raw_sample_' + str(FRAC) + '.pkl'):
            print(samp_path+' already genetaed sampled data!')
            # sys.exit(0)
            user_sub = pd.read_pickle(samp_path + '/user_profile_' + str(FRAC) + '.pkl')
            ...
            pd.to_pickle(user_sub,  samp_path  + '/user_profile_' + str(FRAC) + '.pkl')
            pd.to_pickle(sample_sub, samp_path + '/raw_sample_'   + str(FRAC) + '.pkl')
        ...
        pd.to_pickle(ad,  samp_path + '/ad_feature_enc_' + str(FRAC) + '.pkl')
        pd.to_pickle(log, samp_path + '/behavior_log_pv_user_filter_enc_btag_' + str(FRAC) + '.pkl')

        log.drop(columns=['btag'], inplace=True)    # 去掉 btag 列，直接在原始数据上drop，返回空。 inplace=false返回copy（默认）
        pd.to_pickle(log, samp_path + '/behavior_log_pv_user_filter_enc_' + str(FRAC) + '.pkl')
        print("==0: gen ",samp_path + '/behavior_log_pv_user_filter_enc_btag_' + str(FRAC) + '.pkl and ' + \
            samp_path + '/behavior_log_pv_user_filter_enc_' + str(FRAC) + '.pkl')
