...
class AttentionSequencePoolingLayer(Layer):
    ...
    def call(self, inputs, mask=None, training=None, **kwargs):
        ...
        if self.supports_masking:
            ...
            key_masks = tf.sequence_mask(keys_length, hist_len) # 对于填充了0的张量，添加mask，计算损失函数时去掉无用损失

        attention_score = self.local_att([queries, keys], training=training) # shape=(?,5,1)

        outputs = tf.transpose(attention_score, (0, 2, 1)) # shape=(?,1,5)
        ...
        outputs = tf.where(key_masks, outputs, paddings) # 根据key_masks中的布尔值，T时取outputs中的值，F取padding

        if self.weight_normalization:
            outputs = tf.nn.softmax(outputs)

        if not self.return_score:
            outputs = tf.matmul(outputs, keys) # 矩阵乘法
