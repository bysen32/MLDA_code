...
if __name__ == "__main__":
    ...
    pd.to_pickle(model_input, model_path +'dien_input_' + 
                str(FRAC) + '_' + str(DIN_SESS_MAX_LEN) + '.pkl')
    pd.to_pickle(data['clk'].values, model_path + '/dien_label_' +
                str(FRAC) + '_' + str(DIN_SESS_MAX_LEN) + '.pkl')
    try:
        pd.to_pickle({'sparse': sparse_feature_list, 'dense': dense_feature_list},
                     model_path + '/dien_fd_' + str(FRAC) + '_' + str(DIN_SESS_MAX_LEN) + '.pkl', )
    except:
        pd.to_pickle({'sparse': sparse_feature_list, 'dense': dense_feature_list},
                     model_path + '/dien_fd_' + str(FRAC) + '_' + str(DIN_SESS_MAX_LEN) + '.pkl', )

        print("gen dien input of samp0" + str(sub) + " done")
