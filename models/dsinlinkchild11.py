...
from tensorflow.python.keras.layers import (Concatenate, Dense, Embedding, Reshape, Multiply, Add,Average,
...
class CosineLayer():
    def __call__(self, x1, x2):
        def _cosine(x):
            ...
            return dot1 / max_
#         output_shape = (1,)
        value = Lambda(_cosine)([x1, x2])
        return value


def DSINCHILD11(feature_dim_dict, sess_feature_list, embedding_size=8, sess_max_count=5, sess_len_max=10, bias_encoding=False,
...
# cate_emb_len: 6= (1+4+1)
    ...
    for di in range(max_lk_len):
        ...
        b_link.append(bi)  # max_lk_len 个 shape(?,1,11) brand_lk tensor list
        c_link.append(ci)  # max_lk_len 个 shape(?,1,11)  cate_lk tensor list
    blk_emb_initial = concat_fun(b_link, 1) # shape(?,max_lk_len,11)
    clk_emb_initial = concat_fun(b_link, 1) # shape(?,max_lk_len,11)
    
    #### split
    # id layer, shape(?, max_lk_len)
    blk_id = Lambda(lambda x: x[:, :, 0])(blk_emb_initial) # shape(?,5) 直接用这种list会导致维度缩减，tf.slice不会
    clk_id = Lambda(lambda x: x[:, :, 0])(clk_emb_initial)
    # child id
    blk_child = Lambda(lambda x: x[:, :, 1:6])(blk_emb_initial) # shape=(?, 5, 5)
    clk_child = Lambda(lambda x: x[:, :, 1:6])(clk_emb_initial)
    # take number feat of origin emb
    blk_num_emb = Lambda(lambda x: x[:, :, -5:])(blk_emb_initial) # shape=(?, 5, 5)
    clk_num_emb = Lambda(lambda x: x[:, :, -5:])(clk_emb_initial)

    #### EMBEDDING # shape(?,5,20)
    # ref get_embedding_vec_list: shape(?, max_lk_len) --> shape(?, max_lk_len, 4)
    blk_id_emb = sparse_embedding_dict['brand'](blk_id)   # shape(?,5,4)
    clk_id_emb = sparse_embedding_dict['cate_id'](clk_id)
    ## blk_child_emb_list: max_lk_len 个 shape(?,child,4)
    # blk_child_emb_list = Lambda(gen_child_emb_list)(blk_child, sparse_embedding_dict, max_lk_len, mode='brand')
    # clk_child_emb_list = Lambda(gen_child_emb_list)(clk_child, sparse_embedding_dict, max_lk_len, mode='cate_id')
    blk_child_emb_list = []
    clk_child_emb_list = []
    
    for i in range(max_lk_len):
        blk_child_emb_list.append(Lambda(lambda x:sparse_embedding_dict['brand'](x[:,i,:]))(blk_child))
        clk_child_emb_list.append(Lambda(lambda x:sparse_embedding_dict['cate_id'](x[:,i,:]))(clk_child))

    ## flatten and reshape to max_lk_len个(?,1,20)
    blk_child_emb_flat=[]
    clk_child_emb_flat=[]
    for i in range(max_lk_len):
        blk_child_emb_flat.append(Lambda(lambda x:Reshape((1,-1))(Flatten()(NoMask()(x))))(blk_child_emb_list[i])) # max_lk_len 个 shape(?,1,20)
        clk_child_emb_flat.append(Lambda(lambda x:Reshape((1,-1))(Flatten()(NoMask()(x))))(clk_child_emb_list[i]))
    blk_child_emb = concat_fun(blk_child_emb_flat, 1) # shape(?,5,20)
    clk_child_emb = concat_fun(blk_child_emb_flat, 1)
    # ##### concat id_emb, blk_child_emb and origin num emb
    blk_emb_all = concat_fun([blk_id_emb, blk_child_emb, blk_num_emb]) # shape(?,5,4)+(?,5,20)+(?,5,5)
    clk_emb_all = concat_fun([clk_id_emb, clk_child_emb, clk_num_emb])
    ...
    # activation unit
    blk_score = AttentionSequencePoolingLayer(att_hidden_units=(64, 16), weight_normalization=True,
                                               return_score=True, supports_masking=False)(
        [query_emb_brand, blk_id_emb, brand_link_len])
    clk_score = AttentionSequencePoolingLayer(att_hidden_units=(64, 16), weight_normalization=True,
                                               return_score=True, supports_masking=False)(
        [query_emb_cate, clk_id_emb, cate_link_len])


    # # double activation child link
    CHILD_LEN = 5
    b_child_score = gen_cross_score(query_emb_cate, blk_child_emb_list, max_lk_len, CHILD_LEN) #shape=(?, 5)
    c_child_score = gen_cross_score(query_emb_brand, clk_child_emb_list, max_lk_len, CHILD_LEN)


    # # to shape(?,1,max_lk_len)
    b_child_score_15 = Reshape((-1,5))(b_child_score)
    c_child_score_15 = Reshape((-1,5))(c_child_score)
    # # e^x/(sum(e^x))  shape(?,1,5)
    # b_child_score_expNorm = Lambda(lambda x:K.exp(x)  /  K.maximum(K.sum(K.exp(x),axis=-1,keepdims=True), K.epsilon()))(b_child_score_15)
    # c_child_score_expNorm = Lambda(lambda x:K.exp(x)  /  K.maximum(K.sum(K.exp(x),axis=-1,keepdims=True), K.epsilon()))(c_child_score_15)

    double_score_b = Average()([blk_score, b_child_score_15])
    double_score_c = Average()([clk_score, c_child_score_15])
    # # double weighted
    # double_score_b = Lambda(lambda x: Multiply()(x) / K.maximum(K.sum(Multiply()(x),axis=-1,keepdims=True), K.epsilon()))([blk_score, b_child_score_expNorm])
    # double_score_c = Lambda(lambda x: Multiply()(x) / K.maximum(K.sum(Multiply()(x),axis=-1,keepdims=True), K.epsilon()))([clk_score, c_child_score_expNorm])

    # # weighted
    blk_att_representation = Lambda(lambda x: tf.matmul(*x))([double_score_b, blk_emb_all]) # tf.matmul(blk_score, blk_emb_all)
    clk_att_representation = Lambda(lambda x: tf.matmul(*x))([double_score_c, clk_emb_all]) # clk_att_representation = tf.matmul(clk_score, clk_emb_all)
    ...
    deep_input_emb = Concatenate()(
        ...
        Flatten()(clk_att_representation)]) # 60+8+8 = 76 dim    76 + (29) + 29 = 134   ?104=76+28=76+(14+14)
    if len(dense_input) > 0:
        deep_input_emb = Concatenate()(
            [deep_input_emb] + list(dense_input.values())) # 94+1 = 95
...
def gen_cross_score(query_emb, bclk_child_emb, max_lk_len, CHILD_LEN):
    child_score_max = []
    queb = Lambda(lambda x:x[:,0,:])(query_emb)#shape(?,4)
    for li in range(max_lk_len):
        cosine=[] # cosine similarity in each link
        for child_i in range(CHILD_LEN):
            bclk_one_child = Lambda(lambda x:x[li][:,child_i,:])(bclk_child_emb)
            cosine.append(CosineLayer()(queb, bclk_one_child))# 5 shape(?,1)
        child_cosine = concat_fun(cosine) # shape=(?, 5) # all child cosine
        child_score_max.append(Lambda(lambda x:K.max(x, axis=-1, keepdims=True))(child_cosine))# K.max(child_cosine,axis=-1, keepdims=True)) # max cosine of links # each iter, add shape(?,1)
        cross_score = concat_fun(child_score_max) # shape(?,5) # cosine of each link
    
    return cross_score


# put child id to embedding
def gen_child_emb_list(lk_child, embedding_dict, max_lk, mode='brand'):
    lk_child_emb_list = []
    for i in range(max_lk):
        # query_emb_brand = Lambda(lambda x: x[1])(query_emb_list)
        lk_child_emb_list.append(Lambda(lambda x:embedding_dict[mode](x[:,i,:]))(lk_child))
    return lk_child_emb_list