...
def DSINNL(feature_dim_dict, sess_feature_list, embedding_size=8, sess_max_count=5, sess_len_max=10,
    ...
    sparse_embedding_dict = {feat.name: Embedding(feat.dimension,  # 保存时为每个特征中的最大值
                             ...
                             enumerate(
                                 feature_dim_dict["sparse"])}  # mask_zero = Ture 仅对 cate_id 和 brand 下的 embedding 有效
    ...
    deep_input_emb = Flatten()(NoMask()(
        deep_input_emb))  # Flatten将输入展平  # NoMask? # <tf.Tensor 'flatten/Reshape:0' shape=(?, 60) dtype=float32>

    # 5 个 sess 的  (None,10,8)  cate_id+brand Tensor,    [<tf.Tensor shape=(?, 10, 8) >, <>, <>, <>, <>]  10=5*2
