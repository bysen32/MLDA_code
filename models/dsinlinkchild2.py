...
from tensorflow.python.keras.layers import (Concatenate, Dense, Embedding, Reshape, Multiply,
...
def DSINCHILD2(feature_dim_dict, sess_feature_list, embedding_size=8, sess_max_count=5, sess_len_max=10, bias_encoding=False,
...
# cate_emb_len: 6= (1+4+1)
    ...
    blk_child_emb_list = gen_child_emb_list(blk_child, sparse_embedding_dict, max_lk_len, mode='brand')
    clk_child_emb_list = gen_child_emb_list(clk_child, sparse_embedding_dict, max_lk_len, mode='cate_id')
    ...
    ##### concat id_emb and origin num emb
    ...
    CHILD_LEN = 5
    b_child_score = gen_cross_score(query_emb_cate, blk_child_emb_list, max_lk_len, CHILD_LEN)
    c_child_score = gen_cross_score(query_emb_brand, clk_child_emb_list, max_lk_len, CHILD_LEN)
    # to shape(?,1,max_lk_len)
    b_child_score = Reshape((-1,5))(b_child_score)
    c_child_score = Reshape((-1,5))(c_child_score)
    # e^x/(sum(e^x))  shape(?,1,5)
    b_child_score_expNorm = Lambda(lambda x:K.exp(x)  /  K.maximum(K.sum(K.exp(x),axis=-1,keepdims=True), K.epsilon()))(b_child_score)
    c_child_score_expNorm = Lambda(lambda x:K.exp(x)  /  K.maximum(K.sum(K.exp(x),axis=-1,keepdims=True), K.epsilon()))(c_child_score)
    ...
    deep_input_emb = Concatenate()(
        ...
        Flatten()(clk_att_representation)]) # 60+8+8+9+9 = 94 dim
