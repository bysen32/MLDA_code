...
def DSINLK6(feature_dim_dict, sess_feature_list, embedding_size=8, sess_max_count=5, sess_len_max=10, bias_encoding=False,
...
# cate_emb_len: 6= (1+4+1)
    ...
    blk_score = AttentionSequencePoolingLayer(att_hidden_units=(64, 16), weight_normalization=True, 
                        return_score=True, supports_masking=False)([query_emb_brand, blk_id_emb, brand_link_len])
    clk_score = AttentionSequencePoolingLayer(att_hidden_units=(64, 16), weight_normalization=True,
                        return_score=True, supports_masking=False)([query_emb_cate, clk_id_emb, cate_link_len])
    # take 2-6 dim of origin emb
    blk_num_emb = Lambda(lambda x: x[:, :, 1:emb_len])(blk_emb_initial)
    clk_num_emb = Lambda(lambda x: x[:, :, 1:emb_len])(clk_emb_initial)
    ## concat id_emb and origin num emb
    blk_emb_all = concat_fun([blk_id_emb, blk_num_emb])
    clk_emb_all = concat_fun([clk_id_emb, clk_num_emb])
    # weighted
    blk_att_representation = Lambda(lambda x: tf.matmul(*x))([blk_score, blk_emb_all]) # tf.matmul(blk_score, blk_emb_all)
    clk_att_representation = Lambda(lambda x: tf.matmul(*x))([clk_score, clk_emb_all]) # clk_att_representation = tf.matmul(clk_score, clk_emb_all)
    ...
    model = Model(inputs=model_input_list, outputs=output) # initial function model
    return model
